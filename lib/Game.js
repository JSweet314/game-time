...
module.exports = class Game {
    ...
    this.enemyMissiles.forEach((enemyMissile, index) => {
      if (enemyMissile.destroyed === false) {
        ...
        if (enemyMissile.percentXY >= 1) {
          ...
          let explosion = new Explosion(enemyMissile.x, enemyMissile.y);
  ...
  updateExplosions(ctx) {
    this.explosions.forEach(explosion => {
      ...
      // let chainExplosion = explosion.detectEnemyMissile(this.enemyMissiles);
      // if (chainExplosion !== null) {
      //   this.explosions.push(chainExplosion);
      // }
